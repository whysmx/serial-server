...
func parseListenerSection(section *ini.Section) (*ListenerConfig, error) {
	...
	if maxClients, err := section.Key("max_clients").Int(); err == nil {
		listener.MaxClients = maxClients
	}
...
func Save(path string, cfg *Config) error {
	...
	for _, listener := range cfg.Listeners {
		...
		if listener.MaxClients != DefaultMaxClients {
			section.Key("max_clients").SetValue(strconv.Itoa(listener.MaxClients))
		}
	}
